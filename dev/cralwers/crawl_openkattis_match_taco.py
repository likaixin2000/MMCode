...
def scrape_kattis_problemset():
    url = "https://open.kattis.com/problems?page={page}"
    
    problems = []
    for i in tqdm.tqdm(range(0, 38)):
      response = requests.get(url)
      soup = BeautifulSoup(response.text, 'html.parser')

      table = soup.find_all('table', {"class": "table2"})[0]

      for row in table.find_all('tr')[1:]:
          cols = row.find_all('td')
          problem_ele = cols[0].find("a")
          problem_name = problem_ele.text
          problem_link = "https://open.kattis.com" + problem_ele['href']

          problem = {
              "problem_id": problem_link.rsplit('/')[-1],
...
def scrape_problem(problem_meta):
    ...
    """
    save_dir = os.path.join("crawled", "openkattis", "problems")
    ...
    problem_name = soup.find('h1', class_='book-page-heading').text

    # Find the div with id "problembody"
    task_div = soup.find('div', class_='problembody')
    ...
    for idx, img in enumerate(images, start=1):
        ...
        img_dir = os.path.join(save_dir, f"ok_{problem_id}", "images")
    ...
    problem_data = {
        ...
        "problem": task_div.text.strip(),
        "taco_id": problem_meta["taco_id"]
    }

    # Save to file
    save_dir = os.path.join(save_dir, f"ok_{problem_id}")
...
if __name__ == "__main__":
    problem_meta = read_problemset_from_file("crawled/openkattis/openkattis.json")
